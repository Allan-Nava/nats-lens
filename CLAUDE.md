...
- **Gate prima di chiudere**: `npm run typecheck` (extension + webview) + `npm test` verdi (stessi check della CI). La webview React usa `tsconfig.webview.json` (JSX+DOM), separato dal tsconfig dell'extension host.
...
- **TDD stretto sul core**: per ogni logica nuova/modificata in `src/core/`, scrivere prima il test e **verificare il RED** (mostrare il test che fallisce) *prima* di implementare, poi portarlo a GREEN. Niente scorciatoie test+implementazione insieme. La UI glue è esente (non testata per scelta).
